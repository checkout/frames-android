...
import com.checkout.sdk.billingdetails.strategy.StateStrategy
...
class StateStrategyTest {
    ...
    private val billingDetails = BillingDetails(state = BillingDetail("Alabama"))

    @Mock
    private lateinit var store: InMemoryStore

    @InjectMocks
    private lateinit var strategy: StateStrategy

    @Test
    fun `given store has an initial value when get initial value read it from the store`() {
        val expectedText = billingDetails.state.value
    ...
    fun `when text changed then store the value as address one`() {
        ...
        val expectedBillingDetails = billingDetails.copy(state = BillingDetail(expectedText))
        given(store.billingDetails).willReturn(billingDetails)
    ...
    fun `given text length shorter than minimum when focus changed then error shown is true`() {
        ...
        given(store.billingDetails).willReturn(BillingDetails(state = BillingDetail(shortText)))
